<?php
    $bonnets = [
     ["Bonnet en laine",10,"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Duis a leo diam. Quisque lorem orci, accumsan quis dolor sed, gravida.", "img/fbonnet.webp"],
     ["Bonnet en laine bio",14,"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Duis a leo diam. Quisque lorem orci, accumsan quis dolor sed, gravida.", "https://img.ltwebstatic.com/images3_pi/2022/06/02/1654148169d80348aa9b40ae1768ef1eb5dc88c3ab_thumbnail_600x.webp"],
     ["Bonnet en laine et cachemire",20,"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Duis a leo diam. Quisque lorem orci, accumsan quis dolor sed, gravida.","https://www.montparel.com/12130-large_default/beechfield-bf045-bonnet-avec-rabat.jpg"],
     ["Bonnet arc-en-ciel",12,"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Duis a leo diam. Quisque lorem orci, accumsan quis dolor sed, gravida.", "https://www.montparel.com/12175-large_default/beechfield-bf045-bonnet-avec-rabat.jpg"],
    ];

    $passwordCorrect = 'hello';
    // $emailShuldHave = "'@', '.com', '.fr',";
    $emailShuldHave = "test@test.test";
